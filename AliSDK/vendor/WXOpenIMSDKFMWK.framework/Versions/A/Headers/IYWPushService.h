...
/// 开启后，APP进入后台后，IM会保持一段时间的连接，消息走LocalPush提示用户；默认不开启
@property (nonatomic, assign) BOOL enableIMLocalPush;

/// 可由外部决定是否发出LocalPush，或者由外部发出LocalPush
typedef BOOL(^YWPushShouldIssueLocalPushBlock)(id<IYWMessage> aMessage, UILocalNotification *aSuggestedNotification, YWIMCore *aIMCore, NSDictionary *aExtra);

@property (nonatomic, copy) YWPushShouldIssueLocalPushBlock shouldIssueLocalPushBlock;
- (void)setShouldIssueLocalPushBlock:(YWPushShouldIssueLocalPushBlock)shouldIssueLocalPushBlock;
