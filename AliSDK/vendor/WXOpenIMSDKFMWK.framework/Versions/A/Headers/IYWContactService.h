...
/// @param autoLoadingAvatar 可以控制不自动加载头像
- (YWProfileItem *)getProfileForPerson:(YWPerson *)person withTribe:(YWTribe *)tribe autoLoadingAvatar:(BOOL)autoLoadingAvatar;
...
/// @param autoLoadingAvatar 可控制是否自动加载头像
- (void)getProfileForPerson:(YWPerson *)person withTribe:(YWTribe *)tribe expireInterval:(NSTimeInterval)interval autoLoadingAvatar:(BOOL)autoLoadingAvatar withProgress:(YWProfileProgressBlock)progressBlock andCompletionBlock:(YWProfileCompletionBlock)completionBlock;
...
/// 获取不同排序和分组模式的DBModel
- (id<IYWDBModel>)DBModelWithListMode:(YWContactListMode)aMode;
/// 获取不同排序和分组模式的FRC对象
...
// 子账号是否复用主账号的 Profile，默认值 NO
@property (nonatomic, assign) BOOL shouldOnlyUseMainAccountProfile;
...
/// 结束同步的通知，Object为IMCore对象
FOUNDATION_EXTERN NSString *const YWContactSyncEndNotification;
...
/// 根据groupName获取分组
- (YWGroup *)fetchGroupWithGroupName:(NSString *)aGroupName;
/// 获取某个person的分组
...
/// 请使用IYWDBService中的performBatchOperationBlock，异步执行。否则可能无法插入成功。

/// 插入一个本地分组，如果已有同名分组，则不会有任何修改，否则新增一个分组，分组id由sdk确定
- (void)externalInsertOrUpdateGroup:(YWGroup *)aGroup;
/// 移除某个本地分组
- (void)externalRemoveGroupWithId:(NSNumber *)aGroupId;

/// 插入某个好友
- (void)externalInsertOrUpdateContact:(YWPerson *)aPerson withGroup:(YWGroup *)aGroup;
/// 移除某个好友
- (void)externalRemoveContact:(YWPerson *)aPerson;


#pragma mark - 淘宝内部接口

- (BOOL)isXiaoErAccount;
/// 在线搜索陌生人
- (void)asyncOnlineSearchPerson:(NSString *)keyword delegate:(id)delegate;
...
/// 使用关键字搜索用户，使用用户名
// 如果希望搜索好友，needIsFriend置为YES，否则可以置为NO
- (YWFetchedResultsController *)fetchedResultsControllerWithSearchKeyword:(NSString *)aKeyword needIsFriend:(BOOL)needIsFriend imCore:(YWIMCore *)imCore __attribute__((deprecated("- (id<IYWDBModel>)DBModelWithSearchKeyword:(NSString *)aKeyword needIsFriend:(BOOL)aNeedIsFriend")));
