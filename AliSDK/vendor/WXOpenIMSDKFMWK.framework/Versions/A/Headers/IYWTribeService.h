...
/**
 *  从本地数据库中获取所有已加入群，返回包含 YWTribe 的数组 
 */
- (NSArray *)fetchAllTribes;

/**
 *  从本地数据库中获取制定类型的已加入群，返回包含 YWTribe 的数组 
 *
 *  @param tribeTypes 其中元素为群类型 YWTribeType 对应的 NSNumber 对象，如 @[@(YWTribeTypeNormal)]
 *
 *  @return 包含 YWTribe 的数组
 */
- (NSArray<YWTribe *> *)fetchTribesOfTypes:(NSArray<NSNumber *> *)tribeTypes;
...
/**
 *  群聊天页面中，是否应该显示群成员的显示名
 */
- (BOOL)shouldShowMemberDisplayNamesInConversationViewForTribe:(YWTribe *)tribe;

/**
 *  群聊天页面中，设置是否应该显示群成员的显示名
 */
- (void)setShouldShowMemberDisplayNamesInConversationView:(BOOL)shouldShow tribe:(YWTribe *)tribe;

/**
 *  获取当前登录用户在群中的角色
 */
- (YWTribeMemberRole)currentLoginedUserRoleInTribe:(YWTribe *)tribe;
 ...
 *  @param keyword  关键字
 */
- (id<IYWDBModel>)DBModelForTribeMessagesWithKeyword:(NSString *)keyword;
 ...
 *  @param keyword  关键字
 */
- (YWFetchedResultsController *)searchTribeMessagesWithKeyword:(NSString *)keyword __attribute__((deprecated("请使用获取YWDBModel的接口")));;
 ...
 *  @return 搜索的frc
 */
- (YWFetchedResultsController *)searchTribeWithKeyword:(NSString *)keyword __attribute__((deprecated("请使用获取YWDBModel的接口")));;
