 ...
 *  耗时操作，建议在次线程执行相关处理
 */
- (NSArray *)fetchAllConversationWithConversationTypes:(NSArray *)aTypes;
...
/// 获取ConversationService中共享的FRC，不建议修改这个FRC的任何属性，包括监听的block。否则会影响全局会话变更的相关回调。
@property (nonatomic, strong, readonly) YWFetchedResultsController *ywFetchedResultsController;
...
#pragma mark - 阿里巴巴二方接口

/**
 *  可以设置有限的过滤条件
 */
- (YWFetchedResultsController *)makeFetchedResultsControllerWithPredicate:(YWPredicate *)predicate;
- (void)resetFetchedResultsController:(YWFetchedResultsController *)aYWFRC withPredicate:(YWPredicate *)aYWPredicate;


/**
 *  DBModel
 */
- (id<IYWDBModel>)DBModelForConversationsWithPredicate:(YWPredicate *)aPredicate;
...
#pragma mark - old APIs, NOT recommended

/// 数据源API，推荐通过makeFetchedResultsController方法，获取独立FRC。
...
/// 是否关闭子会话合并，默认为不关闭，即会合并子会话到一个总的会话
@property (nonatomic, readwrite) BOOL disableConversationGroup;
